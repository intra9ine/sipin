...
function Dashboard() {
  ...
  const formattedData = dashboardData?.schemePayment.map((item) => ({
    ...item,
    label: `Scheme ${item.scheme_id} - Month ${item.month_number}`
  }));
  return (
    <section className="mt-[5.5rem] lg:mt-[1rem]  min-h-screen p-6 bg-[var(--primary-white-hex)]">
    <main className='flex gap-1 flex-col'>
      ...
      <aside className="grid w-full mx-auto gap-10 grid-cols-2 my-6 
...
<Image src={'/icons/users.svg'} width={100} height={100} className='w-[2rem] h-[2rem]' alt='icon'/>
<h1 className="text-xl font-semibold ">{dashboardData?.info.total_users}</h1>
<h6 className="text-sm font-semibold text-[var(--darker-grey-hex)]">Total Users Joined</h6>
        </article> */}
      </aside>
      </section>
